...
bool is_number(const string& s) {
    ...
    return !s.empty() && it == s.end(); //전달 받은 문자열이 숫자로 구성된 문자열인지 검사하는 함수로 수정_12/8_김성수
}
...
} // 숫자가 아닌 다른 문자를 입력하면 숫자를 입력하면 숫자를 입력할 때까지 재입력 요구 수정_12/8_김성수
...
} // 2번기능 구현방법 변경_11/21_김성수(보기 중 자신의 증상 입력받기 -> 증상 체크 형식으로 변경)
...
int main() {
    ...
    cvi.push_back(CVI("접종 후 39도 이상 고열이 있거나 평소와 다른 신체 증상 발생시, 바로 의사의 진료 받기", "1")); // 퀴즈 입력번호 수정_최대현(O,X -> 1,2)
    ...
    cvi.push_back(CVI("접종 후 3주차에 다리가 붓고 가슴통증이 있다면 의료기관을 방문해야 한다. ", "1")); // 퀴즈 추가_12/2_전신현
    ...
    while (true) {
        ...
        case 1: // 코로나 백신 접종 정보!
            ...
            cout << "\n시작화면으로 돌아가려면 숫자1번을 바로 다음 단계를 원하신다면 아무 숫자를 눌러주세요 : "; // 종료가 아닌 시작화면으로 돌아감_12/7_수정 
            num = int_input();
            if (num == 1)
                break;
        ...
        case 2: // 코로나 백신 접종 후 자신에게 나타난 증상!
            ...
            cout << "\n";
            cout << "□□□□□□□□□□□□□□□□□□□□□□□□□□□□\n";
            cout << "\n시작화면으로 돌아가려면 숫자2번을 바로 다음 단계를 원하신다면 아무 숫자를 눌러주세요 : ";  // 종료가 아닌 시작화면으로 돌아감_12/7_수정
            num = int_input();
            if (num == 2)
                break;
        ...
        case 3: // 코로나 백신 접종 후 주의사항에 대한 퀴즈게임!
            ...
            while (true) {
                ...
                cout << "최종점수 :" << score << "점\n";
                cout << "★★▣       주의사항 퀴즈 게임       ▣★★\n"; // UI추가_12/8_최대현
                cout << "★         모든 문제가 끝났습니다.        ★\n"; //점수 기능_12/2_전신현, 최종 점수 알림, 게임 다시 시작기능 추가 수정_12/8_김성수
        ...
        case 4: // 프로그램 종료!
            ...
            cout << "▣〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓▣\n";
            return 0;
        default: // 예외처리_전신현(기능 1~4 외에 다른 숫자 입력 시 예외처리)
            // 예외처리 UI
            cout << "\n";
            cout << "              =================================================================\n";
            cout << "              |                                                               |\n";
